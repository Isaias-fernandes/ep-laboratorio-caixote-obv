...
function aggregate(rows) {
  ...
  return [...by.values()].map(g => ({
    pattern:g.pattern,
    signals:g.signals,
    avgMfePct:+(g.mfe/g.signals).toFixed(3),
    avgMaePct:+(g.mae/g.signals).toFixed(3),
    payoffExcursion:+((g.mfe/g.signals)/Math.max(Math.abs(g.mae/g.signals),0.0001)).toFixed(2),
    retestPct:+(100*g.retest/g.signals).toFixed(1),
    obvConfirmPct:+(100*g.obvConfirmed/g.signals).toFixed(1),
    hitRates:Object.fromEntries(TARGETS.map(t=>[t, +(100*g.targets[t]/g.signals).toFixed(1)])),
    hits:g.targets
...
(async()=>{
  ...
  for (const symbol of SYMBOLS) {
    try {
      ...
      const report = lab.evaluate(candles, '1h', Date.now(), 48, 80);
  ...
  const result = {
    ...
    market:'CRIPTO', timeframe:'H1', candlesPerAsset:1200, horizonCandles:48, minimumScore:80,
    assetsTested:assets.length, errors, totalSignals:allRows.length,
    ranking:summary,
    assets,
    sample: allRows.slice(0,200)
  };
