...
const yahooRange=x=>x==='1m'?'5d':x==='5m'?'5d':x==='1h'?'3mo':'1mo';
...
function track(events,symbol,candles){const now=Date.now();for(const e of events){if(e.symbol!==symbol||e.status==='CLOSED')continue;if(now-e.createdMs>MAX_AGE_MS){e.status='CLOSED';e.closedAt=new Date(now).toISOString();continue}const after=candles.filter(x=>x.t>(e.lastProcessedMs??e.entryTime));if(!after.length)continue;for(const bar of after){if(e.side==='BUY'){e.mfe=Math.max(e.mfe??0,pct(e.entry,bar.h)??0);e.mae=Math.min(e.mae??0,pct(e.entry,bar.l)??0)}else{e.mfe=Math.max(e.mfe??0,-(pct(e.entry,bar.l)??0));e.mae=Math.min(e.mae??0,-(pct(e.entry,bar.h)??0))}e.targets=e.targets||{};e.targetTimes=e.targetTimes||{};for(const t of TARGETS)if((e.mfe??0)>=t&&!e.targets[t]){e.targets[t]=true;e.targetTimes[t]=new Date(bar.t).toISOString()}}const last=after.at(-1);e.lastPrice=last.c;e.lastUpdate=new Date(last.t).toISOString();e.lastProcessedMs=last.t}}
async function analyzeAsset(market,symbol,fetcher){const raw={};const src={};const now=Date.now();for(const interval of TF){const g=await fetcher(symbol,interval);raw[interval]=g.rows.filter(x=>x.t+TF_MS[interval]<=now);src[interval]=g.source}if(raw['1m'].length<200||raw['5m'].length<200||raw['15m'].length<200||raw['30m'].length<200||raw['1h'].length<200)throw Error(`historico insuficiente para EMA200: M1=${raw['1m'].length} M5=${raw['5m'].length} M15=${raw['15m'].length} M30=${raw['30m'].length} H1=${raw['1h'].length}`);const tf={m1:snapshot(raw['1m']),m5:snapshot(raw['5m']),m15:snapshot(raw['15m']),m30:snapshot(raw['30m']),h1:snapshot(raw['1h'])},A=choose(tf,'A'),B=choose(tf,'B'),last=raw['1m'].at(-1);return{market,symbol,updatedAt:new Date().toISOString(),price:last.c,A,B,tf,sources:src,last,m1:raw['1m']}}
...
for(const [market,list,fetcher] of [['CRIPTO',CRYPTO,crypto],['B3',B3,b3]])for(const symbol of list){try{const row=await analyzeAsset(market,symbol,fetcher);latest.push({...row,m1:undefined});ok++;track(events,symbol,row.m1);const before=prevMap.get(`${market}|${symbol}`);for(const model of ['A','B']){const r=row[model],prior=before?.[model];if(['PROMISSOR','FORTE'].includes(r.state)&&(!prior||eventKey(symbol,model,prior)!==eventKey(symbol,model,r))){events.unshift({id:`${market}|${symbol}|${model}|${row.last.t}|${r.side}|${r.state}`,market,symbol,model,state:r.state,side:r.side,score:r.total,entry:row.last.c,entryTime:row.last.t,createdAt:new Date(row.last.t).toISOString(),createdMs:row.last.t,status:'OPEN',mfe:0,mae:0,lastPrice:row.last.c,lastProcessedMs:row.last.t,targets:{},targetTimes:{},categories:r.categorias,context:{m1:row.tf.m1,m5:row.tf.m5,m15:row.tf.m15,m30:row.tf.m30,h1:row.tf.h1},sources:row.sources})}}
...
const out={schemaVersion:2,updatedAt:new Date().toISOString(),scope:{crypto:CRYPTO.length,b3:B3.length,timeframes:TF,targetsPct:TARGETS,eventMaxAgeDays:7,closedCandlesOnly:true},rules:{A:'Antecipacao M1/M5/M15 por categorias Tendencia/Momentum/Fluxo/Volatilidade/Estrutura',B:'Confirmacao M15/M30/H1 por categorias; ADX>=20 na volatilidade',event:'Novo evento em transicao para PROMISSOR ou FORTE; acompanhamento ate 7 dias',tracking:'MFE/MAE percorre todos os candles M1 fechados novos entre coletas'},universe:{total:CRYPTO.length+B3.length,analyzed:ok,errors:errors.length},latest,events:events.slice(0,MAX_EVENTS),pairs:pairs.slice(0,3000),errors:errors.slice(0,200)};fs.mkdirSync(path.dirname(OUT),{recursive:true});fs.writeFileSync(OUT,JSON.stringify(out,null,2));console.log(`Concluido ${ok}/${CRYPTO.length+B3.length}; eventos ${events.length}; pares A->B ${pairs.length}`)}
